...
export default function Chatbot() {
  ...
  const [messages, setMessages] = useState([
    {
      from: "bot",
      text: "Hallo! 👋 Ik ben Nikki, de virtuele assistent van ITsPeople. Hoe kan ik u helpen?",
    },
  ]);
  ...
  const handleSend = (text: string) => {
    ...
    // Simulated responses
    setTimeout(() => {
      let response = "Bedankt voor uw vraag! Ik verbind u graag met één van onze experts. U kunt ons bereiken op info@itspeople.nl of +31 (0) 30 22 70 954.";

      if (text.toLowerCase().includes("diensten")) {
        response = "Wij bieden 5 kerndiensten: Business Consultancy, Project Management, Proces Management, Data Management en Testen & QA. Wilt u meer weten over een specifieke dienst?";
      } else if (text.toLowerCase().includes("assessment") || text.toLowerCase().includes("scan")) {
        response = "Goed idee! Wij hebben drie gratis self-assessments: de Innovation Readiness Scan, IT-Governance Scan en NIS2 Readiness Check. Welke past het beste bij uw vraag?";
      } else if (text.toLowerCase().includes("baan") || text.toLowerCase().includes("vacature")) {
        response = "Geweldig dat je interesse hebt! Bij ITsPeople werk je aan uitdagende transformatieprojecten met een team van experts. Bekijk onze vacatures op itspeople.nl/werken-bij of stuur je CV naar hr@itspeople.nl.";
      } else if (text.toLowerCase().includes("contact")) {
        response = "U kunt ons bereiken via:\n📧 info@itspeople.nl\n📞 +31 (0) 30 22 70 954\n📍 Hogeweg 105, Zaltbommel\n\nOf plan direct een vrijblijvend gesprek in!";
      }
        ...
        {open && (
          ...
          >
            ...
            <div className="bg-its-dark p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-white font-bold text-sm">Nikki</p>
                <p className="text-white/50 text-xs">ITsPeople Assistant</p>
              </div>
